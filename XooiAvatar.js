class XooiAvatar {
    ...
    getAvatar (response) {
        if (response && typeof response === 'object') {
            ...
            // Enregistre l'avatar
            if (userId && avatar)
                this.users[userId].avatar = avatar.src;
            // Affiche l'avatar
            if (userId)
     ...
     */
    static getUserId (method = 'fromURL', input) {
        if (input && typeof input === 'string') {
            /** @type {RegExp} */
            let regex;
            if (method === 'fromURL')
                regex = /u=([0-9]+)/;
            else if (method === 'fromDOM')
                regex = /user-id-([0-9+])/;
            if (regex.test(input))
                return Number(input.match(regex)[1]);
        } else {
